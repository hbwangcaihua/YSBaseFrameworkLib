 ...
 *  @param sender
 */
-(void)hudTap:(UIGestureRecognizer*)sender{
    NSLog(@"登陆窗口隐藏HUD");
    [self hide:YES];
    if ([self.delegate respondsToSelector:@selector(hudClickTabClose)]) {
        [self.delegate hudClickTabClose];  //实现delegate可以进行其他处理
    }
}

//
////双击隐藏hud 遍历view
//-(void)doubleHudTap:(UIGestureRecognizer*)sender{
...
//            if ([self.delegate respondsToSelector:@selector(hudDoubleTabClose)]) {
//                [self.delegate hudDoubleTabClose];  //实现delegate可以进行其他处理
//            }
//        }
//    }
//}
...
- (id)initWithView:(UIView *)view isnotclicktohide:(BOOL)isnotclicktohide{
    ...
    if(!isnotclicktohide){
        //登录双击隐藏HUD
        UITapGestureRecognizer *tap1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hudTap:)];
        tap1.numberOfTapsRequired = 1;
        [self addGestureRecognizer:tap1];
    }
    
    //    //双击隐藏HUD
    //    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(doubleHudTap:)];
...
- (id)initWithWindow:(UIWindow *)window {
	return [self initWithView:window isnotclicktohide:NO];
}

- (id)initWithWindowForNotClickHide:(UIWindow *)window {
//     isnotclicktohide:(BOOL)isnotclicktohide
    return [self initWithView:window isnotclicktohide:YES];
}
...
- (void)layoutSubviews {
    ...
    /* 重设宽度 */
    if([delegate respondsToSelector:@selector(resizeLabelWidth)]){
        float resetWidth = [delegate resizeLabelWidth];
        if(resetWidth!=0){
            labelSize.width = resetWidth;
        }
    }
